...
public:
    ...
    bool insert(const Interval<ValueType> &interval)
    {
        ...
        if (interval->start_state.t < node->intervals.front()->start_state.t) {
            createChildNode(node, interval, Position::Left);
            return true;
        } else if (node->intervals.front()->start_state.t < interval->start_state.t) {
        ...
        if (!isNodeHasInterval(node, interval)) {
            auto it = std::lower_bound(node->intervals.begin(), node->intervals.end(), interval,
                                       [] (const Interval<ValueType> &lhs, const Interval<ValueType> &rhs) -> bool { return (lhs->end_state.t < rhs->end_state.t); });

            node->intervals.insert(it, interval);

            if (node->high < interval->end_state.t) {
                node->high = interval->end_state.t;
            }
    ...
    bool remove(const Interval<ValueType> &interval)
    {
        ...
        if (it != node->intervals.cend()) {
            ...
            if (isNodeAboutToBeDestroyed(node)) {
                ...
                if(interval->end_state.t == node->high){
                    node->high = findHighest(node->intervals);
                }

                if(interval->end_state.t == node->highest){
                    removeFixNodeLimits(node);
                }
...
private:
    ...
    struct Node{
        ...
        Node(const Interval<ValueType> &interval, Color col, Node *nill) :
            ...
            left(nill),
            right(nill),
            high(interval->end_state.t),
            lowest(interval->start_state.t),
            highest(interval->end_state.t)
        {
    ...
    void updateNodeLimits(Node *node) const
    {
        ...
        auto lowest = (left != m_nill) ? left->lowest : node->intervals.front()->start_state.t;
    ...
    void subtreeOverlappingIntervals(Node *node, const Interval<ValueType> &interval, bool boundary, Callback &&callback) const
    {
        ...
        if (node->left != m_nill
                && (boundary ? !(node->left->highest < interval->start_state.t) : interval->start_state.t < node->left->highest)) {
            subtreeOverlappingIntervals(node->left, interval, boundary, std::forward<Callback>(callback));
        }

        if (boundary ? !(interval->end_state.t < node->intervals.front()->start_state.t) : node->intervals.front()->start_state.t < interval->end_state.t) {
            for (auto it = node->intervals.rbegin(); it != node->intervals.rend(); ++it) {
                if (boundary ? !((*it)->end_state.t < interval->start_state.t) : interval->start_state.t < (*it)->end_state.t) {
                    callback(*it);
                } else {
                    break;
                }
    ...
    void subtreeInnerIntervals(Node *node, const Interval<ValueType> &interval, bool boundary, Callback &&callback) const
    {
        ...
        if (boundary ? !(node->intervals.front()->start_state.t < interval->start_state.t) : interval->start_state.t < node->intervals.front()->start_state.t) {
            ...
            for (auto it = node->intervals.begin(); it != node->intervals.end(); ++it) {
                if (boundary ? !(interval->end_state.t < (*it)->end_state.t) : (*it)->end_state.t < interval->end_state.t) {
                    callback(*it);
                } else {
                    break;
                }
        ...
        if (node->right != m_nill
                && (boundary ? !(interval->end_state.t < node->right->lowest) : node->right->lowest < interval->end_state.t)) {
    ...
    void subtreeOuterIntervals(Node *node, const Interval<ValueType> &interval, bool boundary, Callback &&callback) const
    {
        ...
        if (node->left != m_nill
                && (boundary ? !(node->left->highest < interval->end_state.t) : interval->end_state.t < node->left->highest)) {
            subtreeOuterIntervals(node->left, interval, boundary, std::forward<Callback>(callback));
        }

        if (boundary ? !(interval->start_state.t < node->intervals.front()->start_state.t) : node->intervals.front()->start_state.t < interval->start_state.t) {
            for (auto it = node->intervals.rbegin(); it != node->intervals.rend(); ++it) {
                if (boundary ? !((*it)->end_state.t < interval->end_state.t) : interval->end_state.t < (*it)->end_state.t) {
                    callback(*it);
                } else {
                    break;
                }
    ...
    void subtreeIntervalsContainPoint(Node *node, const Interval<ValueType> &point, bool boundary, Callback &&callback) const
    {
        ...
        if (boundary ? !(point < node->intervals.front()->start_state.t) : node->intervals.front()->start_state.t < point) {
            for (auto it = node->intervals.rbegin(); it != node->intervals.rend(); ++it) {
                if (boundary ? !((*it)->end_state.t < point) : point < (*it)->end_state.t) {
                    callback(*it);
                } else {
                    break;
                }
