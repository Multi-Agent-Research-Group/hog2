...
/*insert members*/
   ...
   Pairib_ emplace(_Args&&... __args)
		{
            vec_.emplace_back(__args...);
            if(size()==1){
                return Pairib_(vec_.begin(),true);
            }
            if(bNoDuplicates){
                iterator p= lower_bound(vec_.back());
                if(p==end()-1){
                    return Pairib_(p,false);
                }
                if(p==end()||key_compare_(vec_.back(),*p)){
                    //swap
                    auto v( Pairib_(InsertImpl_(p,vec_.back()),true) );
                    vec_.resize(vec_.size()-1);
                    return v;
                }else{
                    vec_.resize(vec_.size()-1); // Duplicate
                    return Pairib_(p,false);
                }
            }else{
                iterator p= upper_bound(vec_.back());
                auto v(Pairib_(InsertImpl_(p, vec_.back()), true));
                vec_.resize(vec_.size() - 1);
                return v;
            }
...
protected:
   ...
   template <class... _Args>
    iterator EmplaceImpl_(iterator p, _Args&&... __args)
        {return vec_.emplace(p,__args...);}
