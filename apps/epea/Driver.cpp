...
#include "Utilities.h"
#include "ProductGraphEnvironment.h"
...
int main(int argc, char ** argv){
  ...
  while(!detectIndependence(solution,group,groups)){
    ...
    for(int j(0); j<G.size(); ++j){
      ...
      if(g.first.size()>1){
        ProductGraphEnvironment<Edge,tDirection,Map2DConstrainedEnvironment> mae(envs);
        TemporalAStar<MAState<Edge>, ProductGraphEnvironment<Edge,tDirection,Map2DConstrainedEnvironment>::MultiAgentAction, ProductGraphEnvironment<Edge,tDirection,Map2DConstrainedEnvironment> > astar;
