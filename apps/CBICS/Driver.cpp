...
std::vector<EnvironmentContainer<xyztLoc,t3DDirection>> environs;
...
UnitSim *sim = 0;
//typedef CBSUnit<xyztLoc,t3DDirection,UnitTieBreaking3D<xyztLoc,t3DDirection>,UnitTimeCAT<xyztLoc,t3DDirection>> CBICSUnit;
//typedef CBSGroup<xyztLoc,t3DDirection,UnitTieBreaking3D<xyztLoc,t3DDirection>,UnitTimeCAT<xyztLoc,t3DDirection>> CBICSGroup;
typedef CBSUnit<xyztLoc,t3DDirection,TieBreaking3D<xyztLoc>,UniversalConflictAvoidanceTable<xyztLoc>> CBICSUnit;
typedef CBSGroup<xyztLoc,t3DDirection,TieBreaking3D<xyztLoc>,UniversalConflictAvoidanceTable<xyztLoc>,Map3dPerfectHeuristic<xyztLoc,t3DDirection>> CBICSGroup;
std::unordered_map<unsigned,CBICSGroup*> groups;
...
void InitHeadless(){
  ace=(Grid3DConstrainedEnvironment*)environs[0].environment.get();
  ...
  CATTieBreaking<xyztLoc>::randomalg=randomalg;
  CATTieBreaking<xyztLoc>::useCAT=useCAT;
  CATTieBreaking<xyztLoc>::agentRadius=agentRadius;
  ...
  if(noid){
    ...
    for(uint16_t i(0); i<num_agents; ++i){
      ...
      groups[i]=new CBICSGroup({i});
      CBICSGroup* group=groups[i];
      rgroups[i]=i;
      if(i==0 && !gui){
        Timer::Timeout func(std::bind(&processSolution, std::placeholders::_1));
        CBICSGroup::timer->StartTimeout(std::chrono::seconds(killtime),func);
...
int MyCLHandler(char *argument[], int maxNumArgs){
  ...
  if(strcmp(argument[0], "-scenfile") == 0)
  {
    ...
    for(auto a: envdata){
      ...
      for(auto e: a){
        ...
        newEnv->GetMapEnv()->setGoal(waypoints[environs.size()].back());
        newEnv->WaitTime(wait);
        environs.emplace_back(e.name,newEnv,new Map3dPerfectHeuristic<xyztLoc,t3DDirection>(map,newEnv),e.threshold,e.weight);
      }
      ++agent;
    }
  ...
  if(strcmp(argument[0], "-extended") == 0)
  {
    ...
    loadExtendedAreaCollisionTable<Vector3D>(Params::array, Params::indices, Params::ivls,
                                             environs[0].environment->branchingFactor(),
    ...
    for(auto a: envdata){
      ...
      for(auto e: a){
        ...
        if(maxcost){
          ...
          environs.emplace_back(e.name,newEnv,nullptr,e.threshold,e.weight);
        }
