...
std::vector<EnvironmentContainer<node_t,int>> environs;
...
UnitSim *sim = 0;
//typedef CBSUnit<node_t,int,UnitTieBreaking3D<node_t,int>,UnitTimeCAT<node_t,int>> CBICSUnit;
//typedef CBSGroup<node_t,int,UnitTieBreaking3D<node_t,int>,UnitTimeCAT<node_t,int>,ICTSAlgorithm<node_t,int>> CBICSGroup;
typedef CBSUnit<node_t,int,TieBreaking3D<node_t>,UniversalConflictAvoidanceTable<node_t>> CBICSUnit;
typedef CBSGroup<node_t,int,TieBreaking3D<node_t>,UniversalConflictAvoidanceTable<node_t>,GraphPerfectHeuristic> CBICSGroup;
std::unordered_map<unsigned,std::unique_ptr<CBICSGroup>> groups;
std::vector<unsigned> rgroups;
std::vector<CBICSUnit*> units;
...
void processSolution(double elapsed){
  ...
  for (unsigned int x = 0; x < solution.size(); x++) {
    cost += environs[x].environment->GetPathLength(solution[x]);
  ...
  fflush(stdout);
  std::cout
    << "elapsed,expanded,generated,planTime,replanTime,collisionTime,expansions,CATcollchecks,collchecks,cost,actions,maxCSet,meanCSet\n";
  if (verify && elapsed > 0)
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    ...
    processSolution(CBICSGroup::timer->EndTimer());
    //if(Params::verbose)
...
void InstallHandlers()
{
  ...
  InstallCommandLineHandler(MyCLHandler, "-time", "-time", "Use time-range constraints");
  ...
  InstallCommandLineHandler(MyCLHandler, "-nolower", "-nolower", "Do mutexprop unconditionally");
  InstallCommandLineHandler(MyCLHandler, "-tightlower", "-tightlower", "Do mutexprop when lower bounds are tight");
  InstallCommandLineHandler(MyCLHandler, "-contloaded", "-contloaded", "Do mutexprop when contingent constraints are loaded from ancestors");
...
void InitGroupParams(CBICSGroup* group){
...
void fillWaypoints(){
  if(waypoints.size()<num_agents){
    ...
    for(int n(0); n<numsubgoals; ++n){
      ...
      while(conflict){
        conflict=false;
        node_t start(((DigraphEnvironment*)environs[0].environment.get())->nodes[rand() % ((DigraphEnvironment*)environs[0].environment.get())->nodes.size()]);
...
void InitHeadless(){
  ace=(DigraphEnvironment*)environs.rbegin()->environment.get();
  ...
  if(noid){
    groups[0].reset(new CBICSGroup({}));
    CBICSGroup* group=groups[0].get();
    ...
    for(int i(0); i<num_agents; ++i){
      ...
      units.push_back(new CBICSUnit(waypoints[i],&environs[i],agentRadius,softEff));
      units[i]->SetColor(rand() % 1000 / 1000.0, rand() % 1000 / 1000.0, rand() % 1000 / 1000.0); // Each unit gets a random color
      environs[i].environment->SetStart(waypoints[i][0]);
    ...
    for(uint16_t i(0); i<num_agents; ++i){
      ...
      groups[i].reset(new CBICSGroup({i}));
      CBICSGroup* group=groups[i].get();
      rgroups[i]=i;
      if(i==0 && !gui){
        Timer::Timeout func(std::bind(&processSolution, std::placeholders::_1));
        CBICSGroup::timer->StartTimeout(std::chrono::seconds(killtime),func);
      ...
      float softEff(.9);
      units.push_back(new CBICSUnit(waypoints[i],&environs[i],softEff));
      units[i]->SetColor(rand() % 1000 / 1000.0, rand() % 1000 / 1000.0, rand() % 1000 / 1000.0); // Each unit gets a random color
      group->AddUnit(units[i]); // Add to the group
      if(Params::verbose)std::cout << "initial path for agent " << i << ":\n";
      if(Params::verbose)
    ...
    for(int j(i+1); j<solution.size(); ++j){
      ...
      while(a < solution[i].size() && b < solution[j].size()) {
        if(collisionCheck3D(solution[i][a-1], solution[i][a], solution[j][b-1], solution[j][b], agentRadius)){
          ...
          //groups[i]=new CBICSGroup(environs,groups[i]->agents);
          //CBICSGroup* toDelete(groups[rgroups[j]]);
          //CBICSGroup* toDelete2(groups[i]);
    ...
    if(sim){
      ...
      if (!paused) {
        ...
        /*std::cout << "Printing locations at time: " << sim->GetSimulationTime() << std::endl;
          ...
          CBICSUnit* c((CBICSUnit*)group.second->GetMember(x));
          xytLoc cur;
          c->GetLocation(cur);
...
int MyCLHandler(char *argument[], int maxNumArgs){
  ...
  InstallCommandLineHandler(MyCLHandler, "-tightlower", "-tightlower", "Do mutexprop when lower bounds are tight");
  InstallCommandLineHandler(MyCLHandler, "-contloaded", "-contloaded", "Do mutexprop when contingent constraints are loaded from ancestors");
  if(strcmp(argument[0], "-nolower") == 0)
  {
    Params::no_lowerbound = true;
    return 1;
  }
  if(strcmp(argument[0], "-tightlower") == 0)
  {
    Params::tight_lowerbound = true;
    return 1;
  }
  if(strcmp(argument[0], "-contloaded") == 0)
  {
    Params::contingent_loaded = true;
    return 1;
  }
  ...
  if(strcmp(argument[0], "-probfile") == 0){
    ...
    while(std::getline(ss, line)){
      ...
      is >> a >> b;
      wpts.push_back(((DigraphEnvironment*)environs[0].environment.get())->nodes[a]);
      wpts.push_back(((DigraphEnvironment*)environs[0].environment.get())->nodes[b]);
      waypoints.push_back(wpts);
      ((DigraphEnvironment*)environs[agents].environment.get())->goal=waypoints[agents][1];
      ((DigraphEnvironment*)environs[agents].environment.get())->StoreGoal(waypoints[agents][1]);
      agents++;
  ...
  if(strcmp(argument[0], "-graph") == 0)
  {
    ...
    while(agent<num_agents){
      ...
      environs.emplace_back("graph",newEnv,new GraphPerfectHeuristic(0,newEnv),0,1.0);
      agent++;
    }
    return 2;
  }
