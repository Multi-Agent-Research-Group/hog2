...
TEST(Map3D, LineOfSight){
  ...
  ASSERT_FALSE(map.LineOfSight(4,0,3,4,7,3));
...
TEST(map3D, LineOfSight){
  Map3D map(16,16,1); // wide open map with border
  map.SetGrid(15,13,2,Map3D::kGround); // Obstacle on the border
  ASSERT_TRUE(map.LineOfSight(Vector3D(4,11,0),Vector3D(11,12.585999965667725,0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(4.29289, 13.2929, 0),Vector3D(13, 13, 0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(13, 12.586, 0),Vector3D(10, 13, 0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(11,12.585999965667725,0),Vector3D(4,11,0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(13, 13, 0),Vector3D(4.29289, 13.2929, 0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(10, 13, 0),Vector3D(13, 12.586, 0)));
  ASSERT_TRUE(map.LineOfSight(Vector3D(10, 13, 0),Vector3D(13, 12.586, 0)));
}

TEST(map3D, LineOfSightReverse){
  Map3D map("files/small_los3.cfg","files/w110n38.dt1");
  Grid3DEnvironment env(&map);
  ASSERT_EQ(env.LineOfSight(std::pair<xyztLoc,xyztLoc>({7,13,0,6.0f},{7,13,0,7.0f}),std::pair<xyztLoc,xyztLoc>({13,8,0,6.0f},{13,7,0,7.0f})),
      env.LineOfSight(std::pair<xyztLoc,xyztLoc>({13,8,0,6.0f},{13,7,0,7.0f}),std::pair<xyztLoc,xyztLoc>({7,13,0,6.0f},{7,13,0,7.0f})));
}

TEST(constraints, hasconflict){
  Map3D map("files/small.map","files/w110n38.dt1");
  Grid3DEnvironment env(&map);
  std::vector<std::vector<xyztLoc>> solution={
      {
          {5,14,0,0.0f},
          {5,13,0,1.0f},
          {5,12,0,2.0f},
  ...
  ChainLOS<xyztLoc> detector({0,1,2},&env);
      ASSERT_TRUE(detector.HasConflict(solution));
}

TEST(constraints, hasconflict2){
  Map3D map("files/small.map","files/w110n38.dt1");
  Grid3DEnvironment env(&map);
  std::vector<std::vector<xyztLoc>> solution={
      {
  ...
  std::vector<std::pair<xyztLoc,xyztLoc>> action={
      {
          {7,13,0,6.0f},
          {7,13,0,7.0f}
      },
  ...
  ChainLOS<xyztLoc> detector({0,1,2},&env);
  do{
    ASSERT_TRUE(detector.HasConflict(action));
  }while(std::next_permutation(action.begin(),action.end()));
  do{
    ASSERT_TRUE(detector.HasConflict(solution));
