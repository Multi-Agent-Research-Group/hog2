...
#ifndef MultiCostLimitedEnvironment_h
#define MultiCostLimitedEnvironment_h
...
class MultiCostLimitedEnvironment : public SearchEnvironment<MAState<state>, std::vector<action> >
{
  public:
    ...
    // Constructor
    MultiCostLimitedEnvironment(std::vector<environment const*> const& base):env(base){
      heuristics.reserve(env.size());
      for(auto const& e:base){
        heuristics.push_back((Heuristic<typename state::first_type> const*)e);
      }
    }
    MultiCostLimitedEnvironment(std::vector<environment const*> const& base, std::vector<Heuristic<typename state::first_type> const*> const& h):env(base),heuristics(h){}
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::generatePermutations(std::vector<std::vector<state>>& positions, std::vector<MAState<state>>& result, int agent, MAState<state>& current){
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::GetSuccessors(const MAState<state> &s, std::vector<MAState<state>> &neighbors) const
{
...
template<typename state, typename action, typename environment>
double MultiCostLimitedEnvironment<state,action,environment>::HCost(const MAState<state> &node1, const MAState<state> &node2) const
{
  double total(0.0);
...
template<typename state, typename action, typename environment>
double MultiCostLimitedEnvironment<state,action,environment>::GCost(MAState<state> const& node1, MAState<state> const& node2) const {
  double total(0.0);
  for(int i(0); i<node1.size(); ++i){
...
template<typename state, typename action, typename environment>
bool MultiCostLimitedEnvironment<state,action,environment>::GoalTest(const MAState<state> &node, const MAState<state> &goal) const
{
...
template<typename state, typename action, typename environment>
double MultiCostLimitedEnvironment<state,action,environment>::GetPathLength(const std::vector<MAState<state>> &sol) const
{
    //double gcost(0.0);
    //if(sol.size()>1)
...
template<typename state, typename action, typename environment>
uint64_t MultiCostLimitedEnvironment<state,action,environment>::GetStateHash(const MAState<state> &node) const
{
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::OpenGLDraw() const
{
  env[0]->OpenGLDraw();
}

template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::OpenGLDraw(const MAState<state> &l) const
{
  unsigned i(0);
  for(auto const& s: l)
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::OpenGLDraw(const MAState<state>& o, const MAState<state> &n, float perc) const
{
  unsigned i(0);
  for(auto const& s: n)
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::OpenGLDraw(const MAState<state> &, const MultiCostLimitedEnvironment<state,action,environment>::MultiAgentAction &) const
{
    //TODO: Implement this
}

template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::GLDrawLine(const MAState<state> &a, const MAState<state> &b) const
{
  int i(0);
  for(auto const& s: a)
...
template<typename state, typename action, typename environment>
void MultiCostLimitedEnvironment<state,action,environment>::GLDrawPath(const std::vector<MAState<state>> &p) const
{
        if(p.size()<2) return;
