...
class AirLandingStrip {
	...
	public: 
		...
		void AddLaunchLocation(airplaneState as) {launch_locations.push_back(lu_s);}

		const std::vector<airplaneState>& GetLandingLocations() {return landing_targets;}
		const std::vector<airplaneState>& GetTakeoffLocations() {return launch_targets;}
		...
		void glDraw() {
		    ...
		    glBegin(GL_LINES);
		      glVertex3f((xval-width/2.0)/(width/2.0),(min_y-width/2.0)/(width/2.0) ,-4.0*z_val/(255.0*80));
		      glVertex3f((xval-width/2.0)/(width/2.0),(max_y-width/2.0)/(width/2.0) ,-4.0*z_val/(255.0*80));
		    glEnd();
...
class AirLandingController {
	private:
		...
		std::unordered_map<uint, std::vector<AirLandingStrip*>> states_ls;
		std::unordered_map<uint, std::vector<AirLandingStrip*>> states_ts;
	public:
		AirLandingController() {}
		bool GetLandingStates(airplaneState current_state, airplaneState &goal_state);
	
		void AddLandingStrip(AirLandingStrip *s);
		void RemovLandingStrip(AirLandingStrip *s);
