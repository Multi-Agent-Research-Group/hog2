...
bool AirLandingController::GetLandingStates(const airplaneState &current_state, std::vector<airplaneState> &goal_states) {
...
bool AirLandingController::GetTakeoffStates(const airplaneState &current_state, std::vector<airplaneState> &launch_states) {
	if (states_ts[current_state].size() == 0)
		return false;
	else for (auto airstrip : states_ts[current_state]) for (s : airstrip->GetTakeoffLocations()) launch_states.push_back(s);
	return true;
}
...
void AirLandingController::AddLandingStrip(AirplaneEnvironment* parent_environment, AirLandingStrip* s) {
	...
	//s->z_val = parent_environment->RedrawLSNormals(s->getX_min(), s->getX_max(), s->getY_min(), s->getY_max());
}
