...
#ifndef PEThetaStar_H
#define PEThetaStar_H
...
template <class state, class action, class environment>
const char *PEThetaStar<state,action,environment>::GetName()
{
	static char name[32];
	sprintf(name, "PEThetaStar[]");
	return name;
}
 ...
 */
template <class state, class action, class environment>
void PEThetaStar<state,action,environment>::GetPath(environment *_env, const state& from, const state& to, std::vector<state> &thePath)
{
	//discardcount=0;
 ...
 */
template <class state, class action, class environment>
bool PEThetaStar<state,action,environment>::InitializeSearch(environment *_env, const state& from, const state& to, std::vector<state> &thePath)
{
 ...
 */
template <class state, class action, class environment>
void PEThetaStar<state,action,environment>::AddAdditionalStartState(state& newState)
{
 ...
 */
template <class state, class action, class environment>
void PEThetaStar<state,action,environment>::AddAdditionalStartState(state& newState, double cost)
{
 ...
 */
template <class state, class action, class environment>
bool PEThetaStar<state,action,environment>::DoSingleSearchStep(std::vector<state> &thePath){
  ...
  for (unsigned int x = 0; x < succ.size(); x++)
  {
    ...
    double newFCost = openNode.g+edgeCost+weight*theHeuristic->HCost(succ[x], goal);	
    ...
    else if (fgreater(newFCost, fCost))
    {
      ...
      if (nextBest == 0)
        nextBest = newFCost;
      else if (fless(newFCost,nextBest))
        nextBest = newFCost;
    }
  }
  if (!keep)
  {
    ...
    openNode.h += nextBest-fCost;
    openNode.reopened = true;
      ...
      case kOpenList:
        {
          ...
          if(update.first && fless(update.second, openClosedList.Lookup(theID).g))
          {
...
std::pair<uint64_t,double> PEThetaStar<state, action,environment>::SetVertex(AStarOpenClosedData<state> const& p){
  ...
  if(!env->LineOfSight(pp.data,p.data)){
    ...
    for(auto const& n:neighbors){
      ...
      if(openClosedList.Lookup(id).reopened || kClosedList==loc){ // Have we seen this before?
 ...
 */
template <class state, class action, class environment>
state PEThetaStar<state, action,environment>::CheckNextNode()
{
 ...
 */
template <class state, class action,class environment>
void PEThetaStar<state, action,environment>::ExtractPathToStartFromID(uint64_t node, std::vector<state> &thePath)
{
  do {
 ...
 */
template <class state, class action, class environment>
void PEThetaStar<state, action,environment>::PrintStats()
{
 ...
 */
template <class state, class action, class environment>
int PEThetaStar<state, action,environment>::GetMemoryUsage()
{
 ...
 */
template <class state, class action, class environment>
bool PEThetaStar<state, action,environment>::GetClosedListGCost(const state &val, double &gCost) const
{
	uint64_t theID;
 ...
 */
template <class state, class action, class environment>
void PEThetaStar<state, action,environment>::OpenGLDraw() const
{
