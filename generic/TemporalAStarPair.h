...
#ifndef TemporalAStarPair_H
#define TemporalAStarPair_H
...
struct TemporalAStarPairCompare {
 ...
 */
template <class state, class action, class environment, class openList=AStarOpenClosed<state, TemporalAStarPairCompare<state>>>
class TemporalAStarPair : public GenericSearchAlgorithm<state,action,environment> {
public:
	TemporalAStarPair():env(0),totalExternalNodesExpanded(nullptr),externalExpansionLimit(INT_MAX),stopAfterGoal(true),verbose(false),
	...
	state CheckNextNode();
	uint64_t GetStateHash(state const& parent, state const& child)
	{
	...
	{
		uint64_t theID;
		openClosedList.Lookup(env->GetStateHash(node), theID);
...
const char *TemporalAStarPair<state,action,environment,openList>::GetName()
{
	static char name[32];
	sprintf(name, "TemporalAStarPair[]");
	return name;
}
 ...
 */
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::GetPath(environment *_env, const state& from, const state& to, std::vector<state> &thePath, unsigned minTime)
{
...
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::GetPath(environment *_env,
 ...
 */
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::GetPaths(environment *_env, const state& from, const state& to, std::vector<std::vector<state>> &paths, double window, double bestf, unsigned minTime)
{
  double nextbestf(0);
 ...
 */
template <class state, class action, class environment, class openList>
double TemporalAStarPair<state,action,environment,openList>::GetNextPath(environment *env, const state& from, const state& to, std::vector<state> &thePath, unsigned minTime)
{
  if(openClosedList.OpenSize() == 0){
...
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::GetPath(environment *_env, const state& from, const state& to, std::vector<action> &path)
{
 ...
 */
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state,action,environment,openList>::InitializeSearch(environment *_env, const state& from, const state& to, std::vector<state> &thePath, unsigned minTime)
{
	thePath.resize(0);
	env = _env;
	//	closedList.clear();
	...
	openClosedList.AddOpenNode(start, GetStateHash(start,start), 0, weight*theHeuristic->HCost(start, goal)*state::TIME_RESOLUTION);
 ...
 */
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state,action,environment,openList>::InitializeSearch(environment *_env,
state const& to,
 ...
 * @date 01/06/08
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::AddAdditionalStartState(state const& newState)
{
 ...
 * @date 09/25/10
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state,action,environment,openList>::AddAdditionalStartState(state const& newState, double cost)
{
 ...
 */
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state,action,environment,openList>::DoSingleSearchStep(std::vector<state> &thePath, unsigned minTime)
{
  if(openClosedList.OpenSize() == 0)
  {
  ...
  {
    uint64_t theID;
    neighborLoc[x]=openClosedList.Lookup(GetStateHash(nn,neighbors[x]), theID);
    neighborID[x]=theID;
  ...
  for (int x = 0; x < nSuccessors; x++)
  {
    ...
    {
      case kClosedList:
        if(verbose)std::cout << "Already Closed ("<<std::hex<<GetStateHash(nn,neighbors[x])<<std::dec<<") " << neighbors[x] << (G+edgeCosts[x]) << "+" << (weight*hCosts[x]) << "=" << (G+edgeCosts[x]+weight*hCosts[x]) << "\n";
      ...
      case kOpenList:
        ...
        break;
      case kNotFound:
        if(verbose)std::cout << "Add node? ("<<std::hex<<GetStateHash(nn,neighbors[x])<<std::dec<<") to open " << neighbors[x] << (G+edgeCosts[x]) << "+" << (weight*hCosts[x]) << "=" << (G+edgeCosts[x]+weight*hCosts[x]) << " ";
        if(fleq(G+edgeCosts[x]+weight*hCosts[x],upperlimit)){
			if(verbose) std::cout << "<="<<upperlimit<<"\n";
          openClosedList.AddOpenNode(neighbors[x],
              GetStateHash(nn,neighbors[x]),
              G+edgeCosts[x],
              weight*hCosts[x],
              nodeid);
        }else{
 ...
 */
template <class state, class action, class environment, class openList>
state TemporalAStarPair<state, action,environment,openList>::CheckNextNode()
{
	uint64_t key = openClosedList.Peek();
 ...
 */
template <class state, class action,class environment,class openList>
void TemporalAStarPair<state, action,environment,openList>::ExtractPathToStartFromID(uint64_t node,
																	 std::vector<state> &thePath)
{
	do {
...
/*template <class state, class action,class environment,class openList>
const state &TemporalAStarPair<state, action,environment,openList>::GetParent(const state &s)
{
	uint64_t theID;
 ...
 */
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state, action,environment,openList>::PrintStats()
{
...
/**
 * Return the amount of memory used by TemporalAStarPair
 * @author Nathan Sturtevant
 * @date 03/22/06
 * 
 ...
 */
template <class state, class action, class environment, class openList>
int TemporalAStarPair<state, action,environment,openList>::GetMemoryUsage()
{
 ...
 * the states
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state, action,environment,openList>::GetClosedListGCost(const state &val, double &gCost) const
{
	uint64_t theID;
...
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state, action,environment,openList>::GetOpenListGCost(const state &val, double &gCost) const
{
	uint64_t theID;
	dataLocation loc = openClosedList.Lookup(env->GetStateHash(val), theID);
	if (loc == kOpenList)
...
template <class state, class action, class environment, class openList>
bool TemporalAStarPair<state, action,environment,openList>::GetClosedItem(const state &s, AStarOpenClosedData<state> &result)
{
	uint64_t theID;
 ...
 */
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state, action,environment,openList>::OpenGLDraw() const
{
 ...
 */
template <class state, class action, class environment, class openList>
void TemporalAStarPair<state, action,environment,openList>::Draw() const
{
	double transparency = 1.0;
	if (openClosedList.size() == 0)
		return;
...
template <class state, class action, class environment, class openList>
std::string TemporalAStarPair<state, action,environment,openList>::SVGDraw() const
{
	std::string s;
	double transparency = 1.0;
	if (openClosedList.size() == 0)
